const utc = new Date();
const NOW = new Date(
  utc.getTime() + utc.getTimezoneOffset() * 60 * 1000 + 3600 * 8 * 1000
);
...
let region;
const raw_url =
  "https://raw.githubusercontent.com/ExpTechTW/API/master/resource/";
const api_url = `https://${URL_MAIN}/`;

axios.get(`${raw_url}region.json`).then(function (res) {
  region = res.data;
  axios.get(`${raw_url}station.json`).then(function (res) {
    station_data = res.data;
    _stations.empty();
    for (const id of Object.keys(station_data)) {
      if (station_data[id].net == "SE-Net") continue;
      const opt_station = $("<option></option>")
        .val(id)
        .html(
          `${id} | ${loc_code_to_string(
            station_data[id].info[station_data[id].info.length - 1].code
          )}`
        );
...
Network.on("change", function () {
  ...
  for (const id of Object.keys(station_data)) {
    if (
      (Network.val() == "MS" && station_data[id].net == "SE-Net") ||
      (Network.val() == "SE" && station_data[id].net == "MS-Net")
    )
      continue;
    const opt_station = $("<option></option>")
      .val(id)
  ...
  fetch(`${api_url}api/v1/trem/wave`, {
    ...
    .then(function (data) {
      if (data.success) {
        ...
        else if (res == "Failed to write file")
          alert("未知錯誤 請聯絡 開發人員!");
