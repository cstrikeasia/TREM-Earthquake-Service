...
async function start() {
  ...
  document.getElementById("EventID").textContent = `EventID:  ${formatTime(
    Number(trem.ID)
  )
    .replaceAll("/", "")
    .replaceAll(":", "")
  ...
  document.getElementById(
    "Source"
  ).textContent = `發表單位:    TREM(${trem.Source})`;
  document.getElementById(
    "Alert_time"
  ).textContent = `檢知時刻:    ${formatTime(Number(trem.ID))}`;
  ...
  for (const eew of trem_eew) {
    ...
    after_eq_time.textContent = eq
      ? Math.round((eew.time - eq.time) / 1000)
      : "未知";
    ...
    dist.textContent = eq
      ? distance(eew.eq.lat, eew.eq.lon)(eq.lat, eq.lon).toFixed(1)
      : "未知";
    ...
    if (!rts_alert && eew.rts) {
      ...
      alert_after_eq_time.textContent = eq
        ? Math.round((eew.time - eq.time) / 1000)
        : "未知";
  ...
  fetch(`https://${URL_MAIN}/api/v1/trem/info`, {
    method: "POST",
    ...
    .then((ans) => {
      for (const ID of station_list) {
        ...
        ax.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.ax.toFixed(4)
            : station.ax.toFixed(2);
        const ay = document.createElement("td");
        ay.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.ay.toFixed(4)
            : station.ay.toFixed(2);
        const az = document.createElement("td");
        az.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.az.toFixed(4)
            : station.az.toFixed(2);
        const vx = document.createElement("td");
        vx.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.vx.toFixed(4)
            : station.vx.toFixed(2);
        const vy = document.createElement("td");
        vy.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.vy.toFixed(4)
            : station.vy.toFixed(2);
        const vz = document.createElement("td");
        vz.textContent =
          station_info[station_id].net == "MS-Net"
            ? station.vz.toFixed(4)
            : station.vz.toFixed(2);
        ...
        link.href = `https://${URL_MAIN}/file/trem_report/${station.ID}.zip`;
        link.target = "_blank";
...
async function searchTrem(id) {
  try {
    const res = await fetch(
      `https://${URL_MAIN}/api/v1/trem/list/${id}?key=${params.key}`
    );
...
async function searchEq(id) {
  try {
    const res = await fetch(
      `https://${URL_MAIN}/api/v2/eq/report/${id}?key=${params.key}`
    );
    return await res.json();
  } catch (err) {
    console.log(err);
...
async function fetchStationInfo() {
  try {
    const res = await fetch(
      "https://raw.githubusercontent.com/ExpTechTW/API/master/resource/station.json"
    );
...
async function fetchRegionInfo() {
  try {
    const res = await fetch(
      "https://raw.githubusercontent.com/ExpTechTW/API/master/resource/region.json"
    );
...
async function fetchTREMEEW(id) {
  try {
    const res = await fetch(`https://${URL_MAIN}/api/v1/trem/report/${id}`);
    return await res.json();
  } catch (err) {
    console.log(err);
  }
