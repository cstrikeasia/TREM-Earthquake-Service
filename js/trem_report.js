...
function eew_table(data) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if (params.dev || trem.Alarm) document.getElementById("body").appendChild(box);
  }

  document.getElementById("currentPage").textContent = currentPage;
  document.getElementById("totalPages").textContent = Math.ceil(data.length / pageSize);
}
...
function filterAndDisplayData() {
  ...
  if (magnitudeFilter) {
    filteredData = current_data.filter(item => int_to_intensity(item.Max) === magnitudeFilter);
  }
